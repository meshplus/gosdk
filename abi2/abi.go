...
var (
	hashResult, _  = hash.NewHasher(hash.KECCAK_256).Hash([]byte("Error(string)"))
	revertSelector = hashResult[:4]
)
...
func (abi ABI) GetMethod(methodSig string) (*Method, error) {
	if method, ok := abi.Methods[methodSig]; ok {
		return &method, nil
	} else {
		if !strings.HasSuffix(methodSig, ")") {
			if method, ok1 := abi.Methods[methodSig+"()"]; ok1 {
				return &method, nil
			}
		}
