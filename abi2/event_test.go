...
func TestEventId(t *testing.T) {
	balanceHash, _ := hash.NewHasher(hash.KECCAK_256).Hash([]byte("Balance(uint256)"))
	checkHash, _ := hash.NewHasher(hash.KECCAK_256).Hash([]byte("Check(address,uint256)"))
	var table = []struct {
		definition   string
	...
	}{
		{
			...
			expectations: map[string]common.Hash{
				"Balance": common.BytesToHash(balanceHash),
				"Check":   common.BytesToHash(checkHash),
			},
		},
	}
