...
// Deprecated: please use abi.Pack function
func (abi ABI) Encode(funcName string, args ...interface{}) ([]byte, error) {
	...
	if funcName == "" {
		...
		methodPtr, exist := abi.Methods[funcName]
		if !exist {
...
// Deprecated: please use abi.Unpack function or abi.UnpackIntoInterface
func (abi ABI) Decode(funcName string, result []byte) (interface{}, error) {
	var elem interface{}
	err := abi.UnpackIntoInterface(&elem, funcName, result)
	return elem, err
}
