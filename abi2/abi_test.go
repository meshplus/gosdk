...
func TestMethodSignature(t *testing.T) {
	...
	hashResult, _ := hash.NewHasher(hash.KECCAK_256).Hash([]byte(exp))
	idexp := hashResult[:4]
...
func TestMultiPack(t *testing.T) {
	...
	hashResult, _ := hash.NewHasher(hash.KECCAK_256).Hash([]byte(("bar(uint32,uint16)")))
	sig := hashResult[:4]
...
func TestABI_MethodById(t *testing.T) {
	...
	hashResult, _ := hash.NewHasher(hash.KECCAK_256).Hash(nil)
	if _, err = abi.MethodById(hashResult); err == nil {
...
func TestABI_EventById(t *testing.T) {
	...
	for testnum, test := range tests {
		...
		topic := test.event
		hashResult, _ := hash.NewHasher(hash.KECCAK_256).Hash([]byte(topic))
		topicID := common.BytesToHash(hashResult)
		fmt.Println(topicID.Hex())
		...
		result, _ := hash.NewHasher(hash.KECCAK_256).Hash([]byte("unknownEvent"))
		unknowntopicID := common.BytesToHash(result)
